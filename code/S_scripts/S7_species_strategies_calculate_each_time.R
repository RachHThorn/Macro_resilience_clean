...
# Script: S7_species_strategies_calculate_each_time
...
csr <- read_csv("results/CSR_results_StateFy.csv", show_col_types = FALSE) %>%
...
# T1
comm_by_site_1 <- read_csv("results/DRAGNet_T0_T1_all.csv", show_col_types = FALSE) %>%
  mutate(
...
# T2
comm_by_site_2 <- read_csv("results/DRAGNet_T0_T2_all.csv", show_col_types = FALSE) %>%
  mutate(
    New_taxon = str_replace_all(New_taxon, " ", "_"),
    unique    = paste(site_name, year_trt, trt, block, sep = "_")
  ) %>%
...
# T3
comm_by_site_3 <- read_csv("results/DRAGNet_T0_T3_all.csv", show_col_types = FALSE) %>%
  mutate(
    New_taxon = str_replace_all(New_taxon, " ", "_"),
    unique    = paste(site_name, year_trt, trt, block, sep = "_")
  ) %>%
...
special_1 <- comm_by_site_1 %>%
  mutate(comm_dat = purrr::map(data, safe_get_special, CSRtable = CSRtable)) %>%
  dplyr::select(site_name, comm_dat) %>%
  tidyr::unnest(comm_dat)
special_1$time_period <- "T0-T1"

special_2 <- comm_by_site_2 %>%
  mutate(comm_dat = purrr::map(data, safe_get_special, CSRtable = CSRtable)) %>%
  dplyr::select(site_name, comm_dat) %>%
  tidyr::unnest(comm_dat)
special_2$time_period <- "T0-T2"

special_3 <- comm_by_site_3 %>%
  mutate(comm_dat = purrr::map(data, safe_get_special, CSRtable = CSRtable)) %>%
  dplyr::select(site_name, comm_dat) %>%
  tidyr::unnest(comm_dat)
special_3$time_period <- "T0-T3"

all <- rbind(special_1, special_2, special_3)
names(all)
...
all <- all %>% separate(ID, into = c("new_site", "year_trt", "trt","block"),
...
write_csv(all, "results/community_specialisation_results.csv")
