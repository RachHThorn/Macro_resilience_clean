...
# 1) Calculate Species richness and diversity at the DRAGNet site level for T0 only
# 2) Species native or non native status at the DRAGNet site level for T0 only
# 3) The average total cover per site at the DRAGNet site level for T0 only
...
# read in DRAGNet data and filter for T0 only 
# create a unique variable to id each quadrat
dat <- 
  read_csv("results/DRAGNet_T0_T1_all.csv") %>%
  filter(year_trt == "T0") %>%
  mutate(unique = paste(site_name, year_trt, trt, block, sep = "_")) 
...
spec_mat <- wide %>% dplyr::select(-unique)
id_var <- wide %>% dplyr::select(unique)

# get diversity per quadrat
diversity <- wide %>%
  transmute(
    unique,
    shannon = vegan::diversity(as.matrix(spec_mat), index = "shannon",  MARGIN = 1),
...
# collapse to site × time × trt means
all <- all %>%
  group_by(site_name, trt, year_trt) %>%
  summarise(
    site_time_rich    = mean(richness, na.rm = TRUE),
    site_time_simp    = mean(simpson,  na.rm = TRUE),
    site_time_shan    = mean(shannon,  na.rm = TRUE),
  ...
  dplyr::select(site_name, year_trt, trt, site_time_rich:site_time_invsimp)

# export data
write_csv(all, "results/diversity_metrics_dragnet_T0.csv")

#################################################################################
# 2) Species native or non native status at a site
################################################################################

raw_path    <- "data/full-cover-2025-09-11.csv"   # raw DRAGNet with provenance
...
override_prov <- tibble::tribble(
  ...
  "CEREEP - Ecotron IDF", 'Ranunculus_acris', 'NAT',
  "CEREEP - Ecotron IDF", "Rosa_canina", 'NAT',
  "CEREEP - Ecotron IDF", "Cytisus_scoparius", 'NAT',
  ...
  "Castellanos", "Alternanthera_philoxeroides", 'INT',
  "Domaninek", "Veronica_arvensis", "INT",
  "Fowlers Gap", "Salsola_kali", "INT",
  "Hulun Buir", "Agropyron_cristatum", "NAT",
  "Oak Lake Field Station", "Cirsium_arvense", "INT",
  "Oak Lake Field Station", "Andropogon_gerardii", "NAT",       
  "Oak Lake Field Station",  "Trifolium_pratense", "INT",
  "Oak Lake Field Station", "Lactuca_serriola", "INT",
  "Piedmont Prairie", "Rhus_copallinum", "NAT",       
  "Piedmont Prairie", "Carduus_nutans", "INT",       
  "Saihanba", "Fragaria_vesca", "INT",
  "Temple", "Lactuca_serriola", "INT")
...
unique(new$new_provenance)
new %>% filter(new_provenance == "NULL")

# Quick checks
message("Distinct taxa: ", dplyr::n_distinct(new$New_taxon))   # e.g., 84
message("Distinct sites: ", dplyr::n_distinct(new$site_name))  # e.g., 49

ggplot(new, aes(new_provenance)) + geom_bar() +
  labs(title = "Provenance (after overrides)", x = "new_provenance", y = "count")

write_csv(new, "results/species_provenance_T0.csv")
...
write_csv(data, "results/total_cover_T0.csv")
