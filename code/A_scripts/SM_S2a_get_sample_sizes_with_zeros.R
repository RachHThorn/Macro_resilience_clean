...
dat_1 <- dat_1 %>% dplyr::select(site_name, New_taxon, taxon_site, trt, year_trt, block, new_max_cover) %>%
...
samples_1 <- zero_counts %>% left_join(all_counts) %>%
  ...
  dplyr::select(trt, year_trt, zero_counts, non_zero_counts, all_counts)

props_1 <- samples_1 %>% 
  mutate(zero_prop = zero_counts/all_counts) %>%
  mutate(non_zero_prop = 1-zero_prop) %>%
  dplyr::select(trt, year_trt, all_counts, non_zero_prop, zero_prop) %>%
...
dat_2 <- dat_2 %>% 
  dplyr::select(site_name, New_taxon, taxon_site, trt, year_trt, block, new_max_cover) %>%
  group_by(site_name, New_taxon, taxon_site, trt, block) %>%
  pivot_wider(names_from = "year_trt", values_from = "new_max_cover", values_fn = mean) %>% 
  rowwise() %>%
...
ggsave("figures/SM_S2a_sample_size_zeros.jpeg", all, height = 14, width = 12)
